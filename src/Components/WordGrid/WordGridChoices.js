...
function WordGridChoices({ words, onWordClick }) {
  ...
  const handleDragStart = (e, word) => {
    ...
    const data = JSON.stringify({ word, image: images[word] });
    e.dataTransfer.setData('application/json', data); // Use 'application/json' MIME type for structured data
    setDraggedWord(word); // Set the word as being dragged
  };

  const handleTouchStart = (e, word) => {
    // This will track which word is being touched
    setDraggedWord(word);
  };
  ...
  const handleTouchEnd = () => {
    // When touch ends, apply the dragged word to the selected words
    if (draggedWord) {
      handleWordClick(draggedWord); // Trigger the same logic as clicking a word
    }
        ...
        <div className="selected-words">
          {selectedWords.map((word, index) => (
            <div
              ...
              className="selected-word"
              draggable
              onDragStart={(e) => handleDragStart(e, word)} // Enable dragging for desktop
              onTouchStart={(e) => handleTouchStart(e, word)} // Handle touch start for touch devices
              onTouchMove={handleTouchMove} // Prevent scrolling while dragging on touch devices
              onTouchEnd={handleTouchEnd} // Handle touch end for touch devices
            >
              {word ? (
                <>
                  <img
